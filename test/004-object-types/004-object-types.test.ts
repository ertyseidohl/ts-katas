...
test("test_1 returns an object with a number, string, and boolean", async () => {
  assert.equal(typeof test_1().n, "number");
  assert.equal(typeof test_1().s, "string");
  assert.equal(typeof test_1().b, "boolean");
});

test("test_2 returns an object matching the interface I", async () => {
